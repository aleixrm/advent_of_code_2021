...
func playBingo(numSequence []string, hits hitsBoard, locations numberLocations) (bool, string, int) {
	finishedBoards := make([]bool, len(hits))
	lastWinResult := -1
	lastWinNum := -1
	for _, num := range numSequence {
		for _, loc := range locations[num] {
			hits[loc.board][loc.row][loc.col] = -1
			if !finishedBoards[loc.board] &&
				(checkRow(hits, loc.row, loc.board) || checkCol(hits, loc.col, loc.board)) {
				lastWinSum := unmarkedSum(hits, loc.board)
				var err error
				lastWinNum, err = strconv.Atoi(num)
				check(err)
				lastWinResult = lastWinNum * lastWinSum
				finishedBoards[loc.board] = true
			}
		}
	}
	if len(finishedBoards) > 0 {
		return true, strconv.Itoa(lastWinNum), lastWinResult
	}
	return false, "", -1
}
